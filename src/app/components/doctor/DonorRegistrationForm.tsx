...
const DISTRICTS = [
  'بني سويف', 'ناصر', 'الواسطى', 'ببا', 'الفشن', 'إهناسيا', 'سمسطا', 'نزلة',
];
...
export default function DonorRegistrationForm() {
  ...
  if (success) {
    ...
    const donationTypeLabel: Record<string, string> = { whole: 'دم كامل', plasma: 'بلازما', platelets: 'صفائح دموية' };
          ...
          <div className="w-full max-w-md mx-auto">
            ...
            <div className="bg-white border border-gray-100 rounded-2xl p-4 mb-4 shadow-sm space-y-3">
              <div className="flex items-center justify-between">
                <span className="text-gray-500" style={{ fontSize: '12px', fontWeight: 600 }}>اسم المتبرع</span>
                <span className="text-gray-900" style={{ fontSize: '13px', fontWeight: 700 }}>{form.name}</span>
              </div>
              <div className="h-px bg-gray-100" />
              <div className="flex items-center justify-between">
                <span className="text-gray-500" style={{ fontSize: '12px', fontWeight: 600 }}>فصيلة الدم</span>
                <span className="text-red-700 font-mono" style={{ fontSize: '14px', fontWeight: 800 }}>{form.bloodType || '—'}</span>
              </div>
              <div className="h-px bg-gray-100" />
              <div className="flex items-center justify-between">
                <span className="text-gray-500" style={{ fontSize: '12px', fontWeight: 600 }}>الحالة</span>
                <span
              ...
              {form.rejectionReason && (
                <>
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <span className="text-gray-500 flex-shrink-0" style={{ fontSize: '12px', fontWeight: 600 }}>السبب</span>
                    <span className="text-gray-700 text-right" style={{ fontSize: '12px' }}>{form.rejectionReason}</span>
                  </div>
                </>
              )}
              {isDeferred && form.deferredUntil && (
                <>
                  <div className="h-px bg-gray-100" />
                  <div className="flex items-center justify-between">
                    <span className="text-gray-500" style={{ fontSize: '12px', fontWeight: 600 }}>موجل حتى</span>
                    <span className="text-orange-700 font-mono" style={{ fontSize: '13px', fontWeight: 700 }}>{form.deferredUntil}</span>
                  </div>
                </>
              )}
            </div>
            ...
            <div className="flex gap-3">
              <button
                onClick={() => { setSuccess(false); setForm(initialForm); setBagVolume('400'); }}
              ...
              <button
                ...
                style={{ background: 'linear-gradient(135deg, #374151, #1f2937)', fontSize: '14px', fontWeight: 700 }}
              >
      ...
      <div className="min-h-[60vh] flex items-center justify-center py-6">
        <div className="w-full max-w-lg mx-auto">
          ...
          <div className="text-center mb-5">
            ...
            <h2 className="text-gray-900 mb-1" style={{ fontSize: '20px', fontWeight: 800 }}>مراجعة وتأكيد بيانات التبرع</h2>
            <p className="text-gray-400" style={{ fontSize: '13px' }}>يرجى مراجعة البيانات أدناه قبل الإرسال للمختبر</p>
          </div>
          ...
          <div className="p-4 bg-gradient-to-l from-green-50 to-emerald-50 border-2 border-green-200 rounded-2xl mb-4 flex items-center justify-between">
            <div>
              <p className="text-gray-500 mb-0.5" style={{ fontSize: '11px', fontWeight: 600 }}>رمز العينة (مولّد تلقائياً)</p>
              <p className="text-green-700 font-mono" style={{ fontSize: '22px', fontWeight: 800, letterSpacing: '0.5px' }}>{donorCode}</p>
            </div>
          ...
          <div className="bg-white border border-gray-100 rounded-2xl p-4 mb-4 shadow-sm">
            <p className="text-gray-500 mb-3 flex items-center gap-1.5" style={{ fontSize: '12px', fontWeight: 700 }}>
              <Activity className="w-3.5 h-3.5 text-green-600" /> البيانات الطبية (مُعبَّأة تلقائياً — للقراءة فقط)
            </p>
            <div className="grid grid-cols-3 gap-3">
              <div className="p-3 bg-red-50 border border-red-100 rounded-xl text-center">
                <p className="text-gray-400 mb-1" style={{ fontSize: '10px', fontWeight: 600 }}>فصيلة الدم</p>
                <p className="text-red-700 font-mono" style={{ fontSize: '20px', fontWeight: 900 }}>{form.bloodType || '—'}</p>
              </div>
              <div className="p-3 bg-blue-50 border border-blue-100 rounded-xl text-center">
                <p className="text-gray-400 mb-1" style={{ fontSize: '10px', fontWeight: 600 }}>نوع التبرع</p>
                <p className="text-blue-700" style={{ fontSize: '13px', fontWeight: 700 }}>{donationTypeLabel[form.donationType] || '—'}</p>
              </div>
              <div className="p-3 bg-green-50 border border-green-100 rounded-xl text-center">
                <p className="text-gray-400 mb-1" style={{ fontSize: '10px', fontWeight: 600 }}>حالة التأهل</p>
                <p className="text-green-700" style={{ fontSize: '13px', fontWeight: 700 }}>✅ مؤهل</p>
              </div>
            </div>
          </div>
            ...
            <div className="flex items-center gap-3">
              <div className="flex-1 relative">
                <input
                  ...
                  pattern="[0-9]*"
                  value={bagVolume}
                  onChange={e => setBagVolume(e.target.value.replace(/[^0-9]/g, ''))}
                  onBlur={() => { if (!bagVolume || +bagVolume <= 0) setBagVolume('400'); }}
                ...
                <span className="absolute left-3 top-1/2 -translate-y-1/2 text-red-400 pointer-events-none"
                  style={{ fontSize: '13px', fontWeight: 600 }}>مل</span>
              </div>
              <div className="flex flex-col gap-1.5">
                {['400', '450', '500'].map(v => (
                  <button key={v} type="button" onClick={() => setBagVolume(v)}
                    ...
                    style={{ fontSize: '11px', fontWeight: 700 }}>{v} مل</button>
                ))}
              </div>
            </div>
            <p className="text-gray-400 mt-2 flex items-center gap-1" style={{ fontSize: '11px' }}>
          ...
          <div className="bg-white border border-gray-100 rounded-2xl p-4 mb-4 shadow-sm">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="p-3 bg-purple-50 border border-purple-100 rounded-xl">
                <p className="text-gray-400 mb-0.5" style={{ fontSize: '10px' }}>التاريخ</p>
                <p className="text-purple-700" style={{ fontSize: '12px', fontWeight: 700 }}>{today}</p>
              </div>
              <div className="p-3 bg-indigo-50 border border-indigo-100 rounded-xl">
                <p className="text-gray-400 mb-0.5" style={{ fontSize: '10px' }}>وقت التبرع</p>
                <p className="text-indigo-700 font-mono" style={{ fontSize: '18px', fontWeight: 800 }}>{form.donationTime || '—'}</p>
              </div>
            </div>
          </div>
          ...
          <div className="p-3 bg-gray-50 border border-gray-100 rounded-xl mb-4 flex items-center justify-center gap-2">
            {form.source === 'walkin'
              ? <><Building2 className="w-4 h-4 text-green-600" /><span className="text-green-700" style={{ fontSize: '13px', fontWeight: 600 }}>تبرع داخل البنك</span></>
              : form.source === 'campaign'
              ? <><Megaphone className="w-4 h-4 text-purple-600" /><span className="text-purple-700" style={{ fontSize: '13px', fontWeight: 600 }}>تبرع من حملة{selectedCampaign ? ` — ${selectedCampaign.title}` : ''}</span></>
              : <><Smartphone className="w-4 h-4 text-blue-600" /><span className="text-blue-700" style={{ fontSize: '13px', fontWeight: 600 }}>حجز من التطبيق</span></>
            }
          </div>
          ...
          <div className="p-3 bg-amber-50 border border-amber-200 rounded-xl mb-4 flex items-start gap-2.5">
            ...
            <p className="text-amber-700" style={{ fontSize: '12px', lineHeight: '1.6' }}>
              بعد الضغط على <strong>«تأكيد وإرسال للمختبر»</strong>، ستُقفَل البيانات وتُرسَل تلقائياً إلى دكتور التحاليل لاستكمال الفحوصات المخبرية.
            </p>
          </div>
          ...
          <div className="flex gap-3">
            ...
            <button
              ...
              style={{ background: 'linear-gradient(135deg, #15803d, #16a34a)', fontSize: '14px', fontWeight: 700, boxShadow: '0 4px 14px rgba(22,163,74,0.30)' }}
            >
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-5">
      {/* Header */}
      <div>
        <h1 className="text-gray-900" style={{ fontSize: '22px', fontWeight: 800 }}>تسجيل متبرع جديد</h1>
        <p className="text-gray-500" style={{ fontSize: '14px' }}>أدخل البيانات الأساسية للمتبرع</p>
      </div>
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-2xl p-4 flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-blue-600" style={{ fontSize: '12px' }}>
              تم جلب بيانات المتبرع تلقائياً. يمكنك تعديلها إذا لزم الأمر، ثم أضف البيانات الطبية في الخطوة التالية.
            </p>
          </div>
        </div>
      )}
      ...
      <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm space-y-5">
        ...
        <div className="flex items-center gap-3 pb-1">
          <div className="flex flex-col items-center gap-1.5">
            <div className={`w-9 h-9 rounded-full flex items-center justify-center transition-all ${step >= 1 ? 'bg-green-600' : 'bg-gray-200'}`}>
              {step > 1
                ? <Check className="w-4 h-4 text-white" />
                : <span className="text-white" style={{ fontSize: '13px', fontWeight: 800 }}>1</span>}
            </div>
            <span className={`${step === 1 ? 'text-green-700' : 'text-gray-400'} text-center`} style={{ fontSize: '11px', fontWeight: 600 }}>البيانات<br />الأساسية</span>
          </div>
          <div className="flex-1 mb-5">
            <div className={`h-0.5 w-full transition-all ${step > 1 ? 'bg-green-500' : 'bg-gray-200'}`} />
          </div>
          <div className="flex flex-col items-center gap-1.5">
            <div className={`w-9 h-9 rounded-full flex items-center justify-center transition-all ${step === 2 ? 'bg-green-600' : 'bg-gray-200'}`}>
              <span className={step === 2 ? 'text-white' : 'text-gray-400'} style={{ fontSize: '13px', fontWeight: 800 }}>2</span>
            </div>
            <span className={`${step === 2 ? 'text-green-700' : 'text-gray-400'} text-center`} style={{ fontSize: '11px', fontWeight: 600 }}>البيانات<br />الطبية</span>
          </div>
        </div>
        ...
        <div>
          ...
          <div className="grid grid-cols-3 gap-3">
            {/* Walk-in */}
            <button type="button" onClick={() => updateField('source', 'walkin')}
              ...
              <div className={`w-8 h-8 rounded-xl flex items-center justify-center flex-shrink-0 ${form.source === 'walkin' ? 'bg-green-600' : 'bg-gray-100'}`}>
                <Building2 className={`w-4 h-4 ${form.source === 'walkin' ? 'text-white' : 'text-gray-400'}`} />
              </div>
              <div className="text-right">
                <p className={form.source === 'walkin' ? 'text-green-700' : 'text-gray-700'} style={{ fontSize: '12px', fontWeight: 700 }}>داخل البنك</p>
                <p className="text-gray-400" style={{ fontSize: '10px' }}>Walk-in</p>
              </div>
            </button>
            ...
            {/* Campaign */}
            <button type="button" onClick={() => updateField('source', 'campaign')}
              ...
              <div className={`w-8 h-8 rounded-xl flex items-center justify-center flex-shrink-0 ${form.source === 'campaign' ? 'bg-purple-500' : 'bg-gray-100'}`}>
                <Megaphone className={`w-4 h-4 ${form.source === 'campaign' ? 'text-white' : 'text-gray-400'}`} />
              </div>
              <div className="text-right">
                <p className={form.source === 'campaign' ? 'text-purple-700' : 'text-gray-700'} style={{ fontSize: '12px', fontWeight: 700 }}>من حملة</p>
                <p className="text-gray-400" style={{ fontSize: '10px' }}>Campaign</p>
              </div>
            </button>
          </div>
          ...
          {form.source === 'campaign' && (
            <div className="mt-3">
              ...
              {errors.campaignId && <p className="text-red-500 mt-1" style={{ fontSize: '11px' }}>{errors.campaignId}</p>}
              {selectedCampaign && (
                <div className="mt-2 p-2.5 bg-purple-50 border border-purple-100 rounded-xl flex items-center justify-between">
                  <span className="text-purple-700" style={{ fontSize: '12px', fontWeight: 600 }}>{selectedCampaign.title}</span>
                  <span className="text-purple-500" style={{ fontSize: '11px' }}>{selectedCampaign.registeredDonors} / {selectedCampaign.targetDonors} متبرع</span>
                </div>
              )}
            </div>
          )}
        ...
        <div>
          ...
          <input value={form.name} onChange={e => updateField('name', e.target.value)} placeholder="مثال: أحمد محمد علي"
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="block text-gray-700 mb-1.5" style={{ fontSize: '13px', fontWeight: 600 }}>الجنس *</label>
            <div className="grid grid-cols-2 gap-2">
              {[['male', 'ذكر'], ['female', 'أنثى']].map(([v, l]) => (
                <button key={v} type="button" onClick={() => updateField('gender', v)}
                  ...
                  style={{ fontSize: '13px', fontWeight: form.gender === v ? 700 : 500 }}>{l}</button>
              ))}
            </div>
            {errors.gender && <p className="text-red-500 mt-1" style={{ fontSize: '11px' }}>{errors.gender}</p>}
          </div>
          <div>
            <label className="block text-gray-700 mb-1.5" style={{ fontSize: '13px', fontWeight: 600 }}>العمر * <span className="text-gray-400">(18-65)</span></label>
            <input type="number" value={form.age} onChange={e => updateField('age', e.target.value)} placeholder="مثال: 28" min="18" max="65"
              ...
              style={{ fontSize: '14px' }} />
            {errors.age && <p className="text-red-500 mt-1" style={{ fontSize: '11px' }}>{errors.age}</p>}
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input
              ...
              onChange={e => updateField('nationalId', e.target.value.replace(/\D/g, '').slice(0, 14))}
              ...
              style={{ fontSize: '14px' }} dir="ltr" />
            {errors.nationalId && <p className="text-red-500 mt-1" style={{ fontSize: '11px' }}>{errors.nationalId}</p>}
          </div>
        </div>
        ...
        <div>
          ...
          <div className="grid grid-cols-3 gap-3">
            {/* Governorate — fixed */}
            <div>
              <label className="block text-gray-500 mb-1.5" style={{ fontSize: '11px', fontWeight: 600 }}>المحافظة</label>
              <div className="w-full px-3 py-3 border border-gray-200 rounded-xl bg-gray-100 text-gray-500 flex items-center gap-1.5"
                style={{ fontSize: '13px' }}>
            ...
            {/* District */}
            <div>
              <label className="block text-gray-500 mb-1.5" style={{ fontSize: '11px', fontWeight: 600 }}>المركز *</label>
              <select
                value={form.district}
            ...
            {/* Area */}
            <div>
              <label className="block text-gray-500 mb-1.5" style={{ fontSize: '11px', fontWeight: 600 }}>المنطقة / الشارع *</label>
              <input
                value={form.area}
                ...
                style={{ fontSize: '13px' }}
              />
              {errors.area && <p className="text-red-500 mt-1" style={{ fontSize: '11px' }}>{errors.area}</p>}
            </div>
          </div>
        </div>
        ...
        <div>
          <label className="block text-gray-700 mb-2" style={{ fontSize: '13px', fontWeight: 600 }}>
            ...
            فصيلة الدم <span className="text-gray-400" style={{ fontWeight: 400 }}>(اختياري)</span>
          </label>
          <div className="grid grid-cols-4 gap-2">
            {BLOOD_TYPES.map(t => (
              <button key={t} type="button" onClick={() => updateField('bloodType', form.bloodType === t ? '' : t)}
                ...
                style={{ fontSize: '15px', fontWeight: form.bloodType === t ? 800 : 500 }}>{t}</button>
            ))}
          </div>
        </div>
        ...
        {/* Donation Type */}
        <div>
          <label className="block text-gray-700 mb-2" style={{ fontSize: '13px', fontWeight: 600 }}>نوع التبرع</label>
          <div className="grid grid-cols-3 gap-3">
            {[['whole', 'دم كامل', '🩸'], ['plasma', 'بلازما', '💧'], ['platelets', 'صفائح', '🔬']].map(([v, l, e]) => (
              <button key={v} type="button" onClick={() => updateField('donationType', v)}
                ...
                <span className={form.donationType === v ? 'text-green-700' : 'text-gray-600'} style={{ fontSize: '12px', fontWeight: form.donationType === v ? 700 : 500 }}>{l}</span>
              </button>
            ))}
          </div>
        </div>
        ...
        {/* ── Medical Screening ── */}
        <div>
          <p className="text-gray-700 mb-3 flex items-center gap-2" style={{ fontSize: '13px', fontWeight: 700 }}>
            <FlaskConical className="w-4 h-4 text-blue-500" />
            الفحص الطبي <span className="text-gray-400" style={{ fontWeight: 400 }}>(اختياري)</span>
          </p>

          {/* Weight + Blood Pressure + Hemoglobin */}
          <div className="grid grid-cols-3 gap-3 mb-3">
            <div>
              <label className="block text-gray-600 mb-1.5 flex items-center gap-1" style={{ fontSize: '12px', fontWeight: 600 }}>
            ...
            <div>
              ...
              <input
                type="number" value={form.hemoglobin}
          ...
          >
            <div className="flex items-center gap-2">
              <AlertTriangle className={`w-4 h-4 ${form.isAllergic ? 'text-orange-500' : 'text-gray-400'}`} />
              <span className={form.isAllergic ? 'text-orange-700' : 'text-gray-600'} style={{ fontSize: '13px', fontWeight: 600 }}>
                لديه حساسية (IsAllergic)
              </span>
            </div>
            <div className={`w-10 h-5 rounded-full transition-all relative ${form.isAllergic ? 'bg-orange-500' : 'bg-gray-200'}`}>
              <div className={`absolute top-0.5 w-4 h-4 bg-white rounded-full shadow transition-all ${form.isAllergic ? 'left-5' : 'left-0.5'}`} />
            </div>
          </button>
        </div>
        ...
        <div>
          <label className="block text-gray-700 mb-2" style={{ fontSize: '13px', fontWeight: 600 }}>
            ...
            الأمراض المزمنة <span className="text-gray-400" style={{ fontWeight: 400 }}>(اختياري — اضغط للتحديد)</span>
          </label>
          <div className="flex flex-wrap gap-2">
            {DISEASES.map(d => (
              <button key={d.id} type="button" onClick={() => toggleDisease(d.id)}
                className={`px-3 py-1.5 rounded-full border-2 transition-all ${form.diseases.includes(d.id)
                  ? 'border-red-400 bg-red-50 text-red-700'
                  : 'border-gray-200 text-gray-600 hover:border-gray-300'}`}
                style={{ fontSize: '12px', fontWeight: form.diseases.includes(d.id) ? 700 : 400 }}>
                {form.diseases.includes(d.id) ? '✕ ' : ''}{d.label}
              </button>
            ))}
          </div>
        ...
        <div>
          ...
          <div className="grid grid-cols-3 gap-3">
            <button type="button" onClick={() => updateField('status', 'eligible')}
              ...
              <div className={`w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0 ${form.status === 'eligible' ? 'bg-green-600' : 'bg-gray-100'}`}>
                <UserCheck className={`w-4 h-4 ${form.status === 'eligible' ? 'text-white' : 'text-gray-400'}`} />
              </div>
              <div className="text-right">
                <p className={form.status === 'eligible' ? 'text-green-700' : 'text-gray-700'} style={{ fontSize: '12px', fontWeight: 700 }}>مؤهل ✅</p>
                <p className="text-gray-400" style={{ fontSize: '10px' }}>يمكنه التبرع</p>
              </div>
            </button>
            <button type="button" onClick={() => updateField('status', 'deferred')}
              ...
              <div className={`w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0 ${form.status === 'deferred' ? 'bg-orange-500' : 'bg-gray-100'}`}>
                <Clock className={`w-4 h-4 ${form.status === 'deferred' ? 'text-white' : 'text-gray-400'}`} />
              </div>
              <div className="text-right">
                <p className={form.status === 'deferred' ? 'text-orange-700' : 'text-gray-700'} style={{ fontSize: '12px', fontWeight: 700 }}>موجل ⏳</p>
                <p className="text-gray-400" style={{ fontSize: '10px' }}>تأجيل مؤقت</p>
              </div>
            </button>
            <button type="button" onClick={() => updateField('status', 'ineligible')}
              ...
              <div className={`w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0 ${form.status === 'ineligible' ? 'bg-red-500' : 'bg-gray-100'}`}>
                <UserX className={`w-4 h-4 ${form.status === 'ineligible' ? 'text-white' : 'text-gray-400'}`} />
              </div>
              <div className="text-right">
                <p className={form.status === 'ineligible' ? 'text-red-700' : 'text-gray-700'} style={{ fontSize: '12px', fontWeight: 700 }}>غير مؤهل ❌</p>
                <p className="text-gray-400" style={{ fontSize: '10px' }}>لا يمكنه التبرع</p>
              </div>
            </button>
          </div>
          ...
          {form.status === 'deferred' && (
            <div className="mt-3 space-y-3 p-4 bg-orange-50 border border-orange-100 rounded-xl">
              <div>
                <label className="block text-orange-700 mb-1.5 flex items-center gap-1" style={{ fontSize: '12px', fontWeight: 600 }}>
                  <AlertTriangle className="w-3.5 h-3.5" /> سبب التأجيل <span style={{ fontWeight: 400 }}>(اختياري)</span>
                </label>
                <textarea
              ...
              <div>
                <label className="block text-orange-700 mb-1.5 flex items-center gap-1" style={{ fontSize: '12px', fontWeight: 600 }}>
                  <Clock className="w-3.5 h-3.5" /> موجل حتى <span style={{ fontWeight: 400 }}>اختياري</span>
                </label>
                <input
                  type="date"
                  value={form.deferredUntil}
                  onChange={e => updateField('deferredUntil', e.target.value)}
          ...
          {form.status === 'ineligible' && (
            <div className="mt-3 space-y-3 p-4 bg-red-50 border border-red-100 rounded-xl">
              <div>
                <label className="block text-red-700 mb-1.5 flex items-center gap-1" style={{ fontSize: '12px', fontWeight: 600 }}>
                  <AlertTriangle className="w-3.5 h-3.5" /> سبب ال��فض <span style={{ fontWeight: 400 }}>(اختياري)</span>
                </label>
                <textarea
                  value={form.rejectionReason}
                  onChange={e => updateField('rejectionReason', e.target.value)}
                  ...
                  style={{ fontSize: '13px' }}
                />
                {errors.rejectionReason && <p className="text-red-500 mt-1" style={{ fontSize: '11px' }}>{errors.rejectionReason}</p>}
              </div>
              <div>
                <label className="block text-red-700 mb-1.5 flex items-center gap-1" style={{ fontSize: '12px', fontWeight: 600 }}>
                  <CalendarDays className="w-3.5 h-3.5" /> محظور حتى (LockoutUntil) <span style={{ fontWeight: 400 }}>اختياري</span>
                </label>
                <input
                  type="date"
                  value={form.lockoutUntil}
                  onChange={e => updateField('lockoutUntil', e.target.value)}
