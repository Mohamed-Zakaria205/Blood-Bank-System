...
function calcEligibility(donor: Donor): EligibilityResult {
  ...
  const eligibleDate = new Date(last.getTime() + wait * 86400000)
    .toISOString()
    .split("T")[0];

  if (daysLeft <= 0)
    return { status: "eligible", daysLeft: 0, daysAgo, eligibleDate };
  if (daysLeft <= 14)
    return { status: "soon", daysLeft, daysAgo, eligibleDate };
...
export default function DoctorEligibility() {
  ...
  const filtered = useMemo(() => {
    return enriched.filter((d) => {
      const matchSearch =
        ...
        d.bloodType.includes(search);
      const matchStatus =
        filterStatus === "all" || d.elig.status === filterStatus;
  ...
  const sendNotification = () => {
    ...
    setSentNotifs(
      (prev) => new Set([...prev, `${notifModal.donor.id}-${notifModal.type}`]),
    );
    setNotifModal(null);
  };
      ...
      <div className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm">
        ...
        <div className="grid grid-cols-4 sm:grid-cols-8 gap-3">
          {BLOOD_TYPES.map((type) => {
            ...
            const typeTotal = enriched.filter(
              (d) => d.bloodType === type,
            ).length;
            return (
              <button
                key={type}
                onClick={() =>
                  setFilterBlood(filterBlood === type ? "all" : type)
                }
                      ...
                      {elig.status === "soon" && (
                        <div className="flex items-center gap-1.5">
                          ...
                          <span
                            ...
                            style={{ fontSize: "12px", fontWeight: 600 }}
                          >
                            يصبح مؤهلاً خلال {elig.daysLeft} يوم — في{" "}
                            {elig.eligibleDate}
                          </span>
                        </div>
                      )}
                      {elig.status === "not_yet" && (
                        <div className="flex items-center gap-1.5">
                          ...
                          <span
                            ...
                            style={{ fontSize: "12px" }}
                          >
                            باقي {elig.daysLeft} يوم — موعد التأهل:{" "}
                            {elig.eligibleDate}
                          </span>
                        </div>
                      )}
                      {elig.status === "deferred" && (
                        <div className="flex items-center gap-1.5">
                          ...
                          <span
                            ...
                            style={{ fontSize: "12px" }}
                          >
                            موجّل حتى {donor.deferredUntil} (باقي{" "}
                            {elig.daysLeft} يوم)
                          </span>
                        </div>
                      )}
                  ...
                  {(elig.status === "eligible" || elig.status === "soon") && (
                    <>
                      ...
                      <button
                        onClick={() =>
                          !hasSentEmergency &&
                          setNotifModal({ donor, type: "emergency" })
                        }
                      ...
                      <button
                        onClick={() =>
                          !hasSentReady &&
                          setNotifModal({ donor, type: "ready" })
                        }
