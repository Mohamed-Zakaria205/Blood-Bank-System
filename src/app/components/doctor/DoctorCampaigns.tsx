...
function buildSlots(startTime: string, endTime: string, duration: string, capacity: string): GeneratedSlot[] {
  ...
  for (let t = startMin; t + dur <= endMin; t += dur) {
    ...
    result.push({ time: fmt(t), endTime: fmt(t + dur), capacity: cap, booked: 0 });
  }
  return result;
}
...
function SlotPreviewCard({ slot, index }: { slot: GeneratedSlot; index: number }) {
  ...
  const borderColor = isFull ? 'border-red-200'    : isNearFull ? 'border-orange-200'    : 'border-green-200';
  ...
  return (
    <div className={`rounded-xl border ${borderColor} ${bgColor} p-2.5 flex flex-col items-center gap-1.5 min-w-0`}>
      {/* Slot number */}
      <span className="text-gray-400" style={{ fontSize: '9px', fontWeight: 700 }}>#{index + 1}</span>

      {/* Time */}
      <span className={`font-mono ${timeColor}`} style={{ fontSize: '12px', fontWeight: 800 }} dir="ltr">
        {slot.time}
      </span>
      <span className="text-gray-400" style={{ fontSize: '9px' }} dir="ltr">↓ {slot.endTime}</span>
      ...
      <div className="flex flex-col items-center gap-0.5">
        ...
        <span className={`px-1.5 py-0.5 rounded-full text-white ${badgeColor}`}
...
export default function DoctorCampaigns() {
  const { user } = useAuth();
  const { slots, cancelAppointment } = useAppointments();
  ...
  // ── Form state ───────────────────────────────────────
  const FORM_DEFAULTS = {
    title: '', location: '', city: 'بني سويف', date: '', targetDonors: '', description: '',
    startTime: '08:00', endTime: '16:00', slotDuration: '30', slotCapacity: '2',
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-gray-900" style={{ fontSize: '22px', fontWeight: 800 }}>حملات التبرع</h1>
          <p className="text-gray-500" style={{ fontSize: '14px' }}>{campaigns.length} حملة مسجلة</p>
        </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        {[
          { label: 'حملات نشطة',  count: campaigns.filter(c => c.status === 'active').length,    color: 'text-emerald-700', bg: 'bg-emerald-50', val: 'active' },
          { label: 'حملات منتهية', count: campaigns.filter(c => c.status === 'completed').length, color: 'text-gray-600',    bg: 'bg-gray-100',   val: 'completed' },
        ].map((s, i) => (
          <button key={i} onClick={() => setFilterStatus(filterStatus === s.val ? '' : s.val)}
            ...
            <div className={s.color} style={{ fontSize: '24px', fontWeight: 800 }}>{s.count}</div>
            <div className="text-gray-600" style={{ fontSize: '12px' }}>{s.label}</div>
          </button>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        {filtered.map(c => {
          ...
          const progressColor = c.status === 'completed' ? 'bg-green-500'
            : pct >= 75 ? 'bg-green-500' : pct >= 40 ? 'bg-yellow-400' : 'bg-red-400';
          const progressTextColor = c.status === 'completed' ? 'text-green-600'
            : pct >= 75 ? 'text-green-600' : pct >= 40 ? 'text-yellow-500' : 'text-red-500';
          return (
            <div key={c.id} className={`bg-white rounded-2xl p-6 border shadow-sm hover:shadow-md transition-all ${isMyCamera ? 'border-green-100' : 'border-gray-100'}`}>
              <div className="flex items-start justify-between mb-3">
                <div className="flex-1">
                  <div className="flex items-center gap-2 flex-wrap mb-1">
                    <span className={`px-2.5 py-0.5 rounded-full ${statusColors[c.status]}`} style={{ fontSize: '11px', fontWeight: 700 }}>{statusLabels[c.status]}</span>
                    {isMyCamera && <span className="px-2 py-0.5 bg-green-100 text-green-700 rounded-full" style={{ fontSize: '10px', fontWeight: 700 }}>حملتي</span>}
                  </div>
                  <h3 className="text-gray-900" style={{ fontSize: '15px', fontWeight: 700 }}>{c.title}</h3>
                </div>
              </div>
              <div className="space-y-2 mb-4">
                <div className="flex items-center gap-2 text-gray-500">
                  <MapPin className="w-4 h-4 flex-shrink-0 text-gray-400" />
                  <span style={{ fontSize: '13px' }}>{c.location} — {c.city}</span>
                </div>
                <div className="flex items-center gap-2 text-gray-500">
              ...
              {c.description && (
                <p className="text-gray-400 mb-4" style={{ fontSize: '12px' }}>{c.description}</p>
              )}
              <div>
                <div className="flex justify-between mb-1.5">
                  <span className="text-gray-600" style={{ fontSize: '12px', fontWeight: 600 }}>الإنجاز</span>
                  <span className={progressTextColor} style={{ fontSize: '12px', fontWeight: 700 }}>{c.registeredDonors} / {c.targetDonors}</span>
                </div>
                <div className="w-full h-2 bg-gray-100 rounded-full overflow-hidden">
                  <div className={`h-full rounded-full transition-all ${progressColor}`} style={{ width: `${Math.min(pct, 100)}%` }} />
                </div>
                <div className="flex items-center justify-between mt-1.5">
                  <span className={`${progressTextColor}`} style={{ fontSize: '11px' }}>{pct}% مكتمل</span>
                  {c.status !== 'completed' && (
                    <div className="flex items-center gap-1 text-gray-400">
                      <TrendingUp className="w-3 h-3" />
                      <span style={{ fontSize: '11px' }}>يتبقى {c.targetDonors - c.registeredDonors} متبرع</span>
                    </div>
                  )}
                </div>
              </div>
                  ...
                  <div className="mt-4 pt-4 border-t border-gray-100">
                    ...
                    >
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-green-700" style={{ fontSize: '13px', fontWeight: 700 }}>المواعيد من التطبيق</span>
                        <span className="px-2 py-0.5 bg-green-600 text-white rounded-full" style={{ fontSize: '11px', fontWeight: 700 }}>
                          {campApts.length}
                        </span>
                      </div>
                          ...
                          return (
                            ...
                            >
                              <div className="flex-shrink-0 text-center w-14">
                                <span className={`font-mono ${isCancelled ? 'text-red-400 line-through' : 'text-green-700'}`}
                                  style={{ fontSize: '13px', fontWeight: 700 }} dir="ltr">{apt.time}</span>
                                <p className="text-gray-400" style={{ fontSize: '10px' }}>{apt.date}</p>
                              </div>
                              <div className="flex-1 min-w-0">
                                <p className={`truncate ${isCancelled ? 'text-red-400 line-through' : 'text-gray-900'}`}
                                  style={{ fontSize: '13px', fontWeight: 600 }}>{apt.donorName}</p>
                                <div className="flex items-center gap-2 mt-0.5">
                                  {apt.donorBloodType && (
                                    <span className={`${isCancelled ? 'text-red-300' : 'text-red-600'}`}
                                      style={{ fontSize: '11px', fontWeight: 700 }}>{apt.donorBloodType}</span>
                                  )}
                                  {isCancelled && apt.cancelledByName && (
                                    <span className="text-red-400" style={{ fontSize: '10px' }}>ألغاه: {apt.cancelledByName}</span>
                                  )}
                                </div>
                              </div>
                              <div className="flex items-center gap-1.5">
                                <span className={`px-2 py-0.5 rounded-full text-white ${
                                  isCompleted ? 'bg-gray-400' : isMissed ? 'bg-orange-500' : isCancelled ? 'bg-red-500' : 'bg-green-600'
                                }`} style={{ fontSize: '10px', fontWeight: 700 }}>
                                  {isCompleted ? 'مكتمل' : isMissed ? 'لم يحضر' : isCancelled ? 'ملغى' : 'محجوز'}
                                </span>
                                {isBooked && (
                                  <>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4"
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between p-6 border-b border-gray-100">
              <h3 className="text-gray-900" style={{ fontSize: '18px', fontWeight: 700 }}>إنشاء حملة تبرع جديدة</h3>
            ...
            {success ? (
              <div className="p-12 text-center">
                ...
                <p className="text-gray-900" style={{ fontSize: '18px', fontWeight: 700 }}>تم إنشاء الحملة بنجاح!</p>
              ...
              <>
                ...
                <div className="p-6 space-y-5">
                  ...
                  {/* ── Basic Info ─────────────────────────── */}
                  <div>
                    <label className="block text-gray-700 mb-1.5" style={{ fontSize: '13px', fontWeight: 600 }}>عنوان الحملة *</label>
                    <input
                      value={form.title}
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label className="block text-gray-700 mb-1.5" style={{ fontSize: '13px', fontWeight: 600 }}>موقع الحملة *</label>
                      <input
                        value={form.location}
                        ...
                        style={{ fontSize: '13px' }}
                      />
                      {errors.location && <p className="text-red-500 mt-1" style={{ fontSize: '11px' }}>{errors.location}</p>}
                    </div>
                    <div>
                      <label className="block text-gray-700 mb-1.5" style={{ fontSize: '13px', fontWeight: 600 }}>المدينة</label>
                      <select
                        value={form.city}
                    ...
                    <div>
                      <label className="block text-gray-700 mb-1.5" style={{ fontSize: '13px', fontWeight: 600 }}>تاريخ الحملة *</label>
                      <input
                        type="date"
                        value={form.date}
                        ...
                        style={{ fontSize: '13px' }}
                      />
                      {errors.date && <p className="text-red-500 mt-1" style={{ fontSize: '11px' }}>{errors.date}</p>}
                    </div>
                    <div>
                      <label className="block text-gray-700 mb-1.5" style={{ fontSize: '13px', fontWeight: 600 }}>العدد المستهدف *</label>
                      <input
                        type="number"
                        value={form.targetDonors}
                        onChange={e => setForm(p => ({ ...p, targetDonors: e.target.value }))}
                        ...
                        style={{ fontSize: '13px' }}
                      />
                      {errors.targetDonors && <p className="text-red-500 mt-1" style={{ fontSize: '11px' }}>{errors.targetDonors}</p>}
                    </div>
                  </div>
                  ...
                  <div className="rounded-2xl border border-green-100 overflow-hidden">
                    ...
                    <div className="flex items-center gap-2.5 px-5 py-3.5 bg-green-50 border-b border-green-100">
                      ...
                      <div>
                        <p className="text-green-800" style={{ fontSize: '13px', fontWeight: 700 }}>جدولة أوقات الحملة</p>
                        <p className="text-green-600" style={{ fontSize: '11px' }}>حدد ساعات العمل ومدة كل فترة زمنية وسعتها</p>
                      </div>
                    </div>
                    ...
                    <div className="p-5 space-y-4">
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        {/* Start time */}
                        <div>
                          <label className="block text-gray-700 mb-1.5" style={{ fontSize: '13px', fontWeight: 600 }}>وقت البداية</label>
                          <input
                            type="time"
                        ...
                        {/* End time */}
                        <div>
                          <label className="block text-gray-700 mb-1.5" style={{ fontSize: '13px', fontWeight: 600 }}>وقت الانتهاء</label>
                          <input
                            type="time"
                            value={form.endTime}
                            ...
                            style={{ fontSize: '13px' }}
                          />
                          {errors.startTime && <p className="text-red-500 mt-1" style={{ fontSize: '11px' }}>{errors.startTime}</p>}
                        </div>
                        {/* Slot duration */}
                        <div>
                          <label className="block text-gray-700 mb-1.5" style={{ fontSize: '13px', fontWeight: 600 }}>مدة الفترة الواحدة</label>
                          <select
                        ...
                        <div>
                          <label className="block text-gray-700 mb-1.5" style={{ fontSize: '13px', fontWeight: 600 }}>
                            سعة كل فترة
                            <span className="text-gray-400 mr-1" style={{ fontWeight: 400 }}>(متبرع)</span>
                          </label>
                          <input
                            type="number"
                            min={1}
                            ...
                            onChange={e => setForm(p => ({ ...p, slotCapacity: e.target.value }))}
                            className="w-full px-4 py-2.5 border border-gray-200 rounded-xl bg-gray-50 text-gray-900 outline-none focus:border-green-400 focus:ring-2 focus:ring-green-100"
                            style={{ fontSize: '13px' }}
                          />
                        </div>
                        ...
                        <div className="space-y-3">
                          ...
                          <div className="flex items-center gap-2 flex-wrap">
                            <div className="flex items-center gap-1.5 px-3 py-1.5 bg-green-100 rounded-xl">
                              ...
                              <span className="text-green-800" style={{ fontSize: '12px', fontWeight: 700 }}>
                            ...
                            <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl"
                              style={{ background: 'linear-gradient(135deg,#dcfce7,#d1fae5)', border: '1px solid #86efac' }}>
                              <Check className="w-3.5 h-3.5 text-green-600" />
                              <span className="text-green-800" style={{ fontSize: '12px', fontWeight: 800 }}>
                            ...
                            <div className="flex items-center gap-1 px-3 py-1.5 bg-gray-100 rounded-xl mr-auto">
                              ...
                              <span className="text-gray-600 font-mono" style={{ fontSize: '11px', fontWeight: 600 }} dir="ltr">
                          ...
                          <div className="flex items-center gap-4">
                            <span className="text-gray-500" style={{ fontSize: '11px' }}>مؤشرات الحالة:</span>
                            {[
                              { color: 'bg-green-500',  label: 'متاح' },
                              { color: 'bg-orange-400', label: 'يو��ك الامتلاء' },
                              { color: 'bg-red-500',    label: 'ممتلئ' },
                            ].map(l => (
                              <div key={l.label} className="flex items-center gap-1.5">
                                <div className={`w-2.5 h-2.5 rounded-full ${l.color}`} />
                                <span className="text-gray-500" style={{ fontSize: '11px' }}>{l.label}</span>
                              </div>
                            ))}
                          </div>
                          ...
                          <div className="flex items-start gap-2 p-3 bg-blue-50 rounded-xl border border-blue-100">
                            ...
                            <p className="text-blue-700" style={{ fontSize: '11px', lineHeight: '1.6' }}>
                              عند امتلاء فترة زمنية بالكامل، تُغلق تلقائياً أمام الحجوزات الجديدة.
                              يمكن للمتبرعين الحجز في الفترات المتاحة عبر التطبيق  .
                            </p>
                          </div>
                        </div>
                      ) : (
