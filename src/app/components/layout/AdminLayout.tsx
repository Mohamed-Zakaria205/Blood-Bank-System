...
import {
  LayoutDashboard, Users, UserCog, Megaphone, Droplets,
...
const navItems = [
  ...
  { path: '/admin/inventory', label: 'مخزون الدم', icon: Droplets },
  ...
  const SidebarContent = () => (
    <div className="flex flex-col h-full">
      ...
      <nav className="flex-1 p-4 space-y-1 overflow-y-auto">
        <p className="text-gray-400 px-3 mb-3" style={{ fontSize: '11px', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em' }}>القائمة الرئيسية</p>
        {navItems.map(item => (
          ...
          >
            {({ isActive }) => (
              <>
                ...
                {item.path === '/admin/inventory' && criticalItems.length > 0 && (
                  <span className={`mr-auto text-xs px-1.5 py-0.5 rounded-full ${isActive ? 'bg-white/20 text-white' : 'bg-red-100 text-red-600'}`}
                    style={{ fontSize: '11px', fontWeight: 700 }}>{criticalItems.length}</span>
                )}
              </>
            )}
          </NavLink>
      ...
      <div className="p-4 border-t border-gray-100">
        <div className="flex items-center gap-3 p-3 rounded-xl bg-gray-50">
          ...
          <div className="flex-1 min-w-0">
            ...
            <span className="inline-block px-2 py-0.5 bg-green-100 text-green-700 rounded-full" style={{ fontSize: '10px', fontWeight: 700 }}>مدير عام</span>
          </div>
            ...
            <div className="flex items-center gap-2">
              {criticalItems.length > 0 && (
                <div className="hidden sm:flex items-center gap-1.5 bg-red-50 border border-red-100 px-3 py-1.5 rounded-lg">
                  ...
                  <span className="text-red-600" style={{ fontSize: '12px', fontWeight: 600 }}>{criticalItems.length} فصائل بمستوى حرج</span>
                </div>
              )}
              ...
              <div className="flex items-center gap-2 pr-2 border-r border-gray-200">
                ...
                <div className="hidden sm:block">
                  ...
                  <span className="inline-block px-1.5 py-0.5 bg-green-100 text-green-700 rounded-full" style={{ fontSize: '10px', fontWeight: 700 }}>مدير عام</span>
                </div>
