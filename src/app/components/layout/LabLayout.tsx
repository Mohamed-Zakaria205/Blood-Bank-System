  ...
  const SidebarContent = () => (
    <div className="flex flex-col h-full">
      ...
      <div className="p-6 border-b border-gray-100">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-gray-500" style={{ fontSize: '11px' }}>قسم التحاليل المخبرية</p>
          </div>
        </div>
      </div>
      ...
      <div className="px-4 py-3">
        <div className="p-3 bg-yellow-50 border border-yellow-100 rounded-xl space-y-2">
          <div className="flex items-center gap-2">
            ...
            <span className="text-yellow-700" style={{ fontSize: '12px', fontWeight: 700 }}>دكتور تحاليل</span>
          </div>
          <div className="flex items-center justify-between pt-1 border-t border-yellow-100">
            <span className="text-yellow-600" style={{ fontSize: '11px' }}>حقائب معلقة</span>
            {pendingCount > 0 ? (
              <span className="px-2 py-0.5 bg-yellow-500 text-white rounded-full animate-pulse" style={{ fontSize: '11px', fontWeight: 700 }}>{pendingCount}</span>
            ) : (
              <span className="px-2 py-0.5 bg-yellow-100 text-yellow-700 rounded-full" style={{ fontSize: '11px', fontWeight: 700 }}>لا شيء</span>
            )}
          </div>
        </div>
      </div>
      ...
      <nav className="flex-1 p-4 space-y-1">
        <p className="text-gray-400 px-3 mb-3" style={{ fontSize: '11px', fontWeight: 600 }}>القائمة</p>
        {navItems.map(item => (
          ...
          >
            {({ isActive }) => (
              <>
                ...
                <span style={{ fontSize: '14px', fontWeight: isActive ? 700 : 500 }}>{item.path === '/lab/samples' ? 'العينات' : item.label}</span>
                {pendingCount > 0 && item.path === '/lab' && (
                  <span className={`mr-auto px-1.5 py-0.5 rounded-full ${isActive ? 'bg-white/20 text-white' : 'bg-yellow-100 text-yellow-700'}`}
                    style={{ fontSize: '11px', fontWeight: 700 }}>{pendingCount}</span>
                )}
              </>
            )}
          </NavLink>
      ...
      <div className="p-4 border-t border-gray-100">
        <div className="flex items-center gap-3 p-3 rounded-xl bg-gray-50">
          <div className="w-9 h-9 bg-green-100 rounded-full flex items-center justify-center flex-shrink-0">
            <span className="text-green-700" style={{ fontSize: '14px', fontWeight: 700 }}>{user?.name?.charAt(2) || 'ت'}</span>
          </div>
          <div className="flex-1 min-w-0">
            <p className="text-gray-900 truncate" style={{ fontSize: '13px', fontWeight: 600 }}>{user?.name}</p>
            <span className="inline-block px-2 py-0.5 bg-green-100 text-green-700 rounded-full" style={{ fontSize: '10px', fontWeight: 700 }}>دكتور تحاليل</span>
          </div>
            ...
            <div className="flex items-center gap-2">
              {pendingCount > 0 && (
                <div className="hidden sm:flex items-center gap-1.5 bg-green-50 border border-green-100 px-3 py-1.5 rounded-lg">
                  ...
                  <span className="text-green-700" style={{ fontSize: '12px', fontWeight: 600 }}>{pendingCount} حقيبة معلقة</span>
                </div>
              )}
              ...
              <div className="flex items-center gap-2 pr-2 border-r border-gray-200">
                <div className="w-8 h-8 bg-green-100 rounded-full flex items-center justify-center">
                  <span className="text-green-700" style={{ fontSize: '12px', fontWeight: 700 }}>{user?.name?.charAt(2) || 'ت'}</span>
                </div>
                <div className="hidden sm:block">
                  <p className="text-gray-900" style={{ fontSize: '13px', fontWeight: 600 }}>{user?.name?.split(' ').slice(0, 2).join(' ')}</p>
                  <span className="inline-block px-1.5 py-0.5 bg-green-100 text-green-700 rounded-full" style={{ fontSize: '10px', fontWeight: 700 }}>دكتور تحاليل</span>
                </div>
