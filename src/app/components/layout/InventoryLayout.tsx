...
export default function InventoryLayout() {
  ...
  const notifications: Notification[] = [
    ...expiredActive.map((b) => ({
      id: `exp-${b.id}`,
      title: `حقيبة ${b.bagCode} منتهية الصلاحية`,
      subtitle: `${b.bloodType} — انتهت في ${b.expiryDate}`,
      icon: <AlertTriangle className="w-4 h-4" />,
      color: 'red' as const,
    })),
    ...nearExpiry.map((b) => ({
      id: `near-${b.id}`,
      title: `حقيبة ${b.bagCode} — قريبة الانتهاء`,
      subtitle: `${b.bloodType} — تنتهي في ${b.expiryDate}`,
  ...
  const SidebarContent = () => (
    <div className="flex flex-col h-full">
      ...
      <div className="px-4 py-3">
        <div
          className={`p-3 border rounded-xl space-y-2 ${totalAlerts > 0 ? 'bg-red-50 border-red-200' : 'bg-green-50 border-green-100'}`}
        >
          <div className="flex items-center gap-2">
            <Package className={`w-4 h-4 ${totalAlerts > 0 ? 'text-red-600' : 'text-green-600'}`} />
            <span
              className={totalAlerts > 0 ? 'text-red-700' : 'text-green-700'}
          ...
          <div
            className="flex items-center justify-between pt-1 border-t border-opacity-30"
            style={{ borderColor: totalAlerts > 0 ? '#fca5a5' : '#86efac' }}
          >
            <span
              className={totalAlerts > 0 ? 'text-red-600' : 'text-green-600'}
              style={{ fontSize: '11px' }}
            >
              {totalAlerts > 0 ? 'تنبيهات الحقائب' : 'الحقائب المتاحة'}
            </span>
            <span
              className={`px-2 py-0.5 rounded-full text-white ${totalAlerts > 0 ? 'bg-red-500 animate-pulse' : 'bg-green-600'}`}
          ...
          >
            {({ isActive }) => (
              <>
                ...
                {item.path === '/inventory/bags' &&
                  (expiredActive.length > 0 || nearExpiry.length > 0) && (
                    <span
                      className={`mr-auto px-1.5 py-0.5 rounded-full ${isActive ? 'bg-white/20 text-white' : 'bg-red-100 text-red-600'}`}
                      style={{ fontSize: '11px', fontWeight: 700 }}
                    >
                      {expiredActive.length + nearExpiry.length}
                    </span>
                  )}
                {item.path === '/inventory/disposal' && disposalAlerts > 0 && (
                  <span
                    className={`mr-auto px-1.5 py-0.5 rounded-full ${isActive ? 'bg-white/20 text-white' : 'bg-orange-100 text-orange-600'}`}
                    style={{ fontSize: '11px', fontWeight: 700 }}
                  >
                    {disposalAlerts}
                  </span>
              ...
              {(expiredActive.length > 0 || nearExpiry.length > 0) && (
                <div className="hidden sm:flex items-center gap-1.5 bg-red-50 border border-red-100 px-3 py-1.5 rounded-lg">
                  ...
                  <span className="text-red-600" style={{ fontSize: '12px', fontWeight: 600 }}>
                    {expiredActive.length + nearExpiry.length} حقيبة تحتاج إجراء
                  </span>
                </div>
              )}
