        ...
        <div className="p-6 space-y-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="text-[#1E293B]" style={{ fontSize: '14px', fontWeight: 600 }}>
                  الفصيلة المطلوبة: {request.bloodType}
                </div>
          ...
          <div>
            ...
            <input
              ...
              min={1}
              className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 focus:outline-none focus:border-[#C62828]"
              style={{ fontSize: '14px' }}
            />
          </div>
          <div>
            ...
            <textarea
              ...
              placeholder="ملاحظات الشحن والتسليم..."
              className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 focus:outline-none focus:border-[#C62828] resize-none"
              style={{ fontSize: '14px' }}
            />
          </div>
          <div className="flex gap-3">
            ...
            <button
              onClick={() => onSubmit(units, note)}
              className="flex-1 py-3 rounded-xl bg-gradient-to-r from-[#C62828] to-[#B71C1C] text-white shadow-md hover:shadow-lg transition-all flex items-center justify-center gap-2"
              style={{ fontSize: '14px', fontWeight: 600 }}
            >
...
export default function AdminEmergency() {
  ...
  if (isError)
    return (
      <ErrorState
        message="فشل في تحميل طلبات الطوارئ، يرجى المحاولة لاحقاً"
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-400 mt-1" style={{ fontSize: '13px' }}>
            إدارة الطلبات الطارئة من المستشفيات
          </p>
        </div>
        {pendingCount > 0 && (
          <div className="flex items-center gap-2 bg-red-50 border border-red-200 px-4 py-2.5 rounded-xl">
            ...
            <span className="text-red-700" style={{ fontSize: '14px', fontWeight: 600 }}>
              {pendingCount} طلبات تنتظر المعالجة
            </span>
          </div>
        )}
      </div>
        ...
        <div className="bg-gradient-to-r from-red-600 to-red-700 rounded-2xl p-5 text-white flex items-center gap-4">
          ...
          <div className="flex-1">
            <div style={{ fontSize: '16px', fontWeight: 700 }}>
              🚨 تنبيه: {criticalCount} حالة طوارئ قصوى!
            </div>
            <div className="text-red-200" style={{ fontSize: '13px' }}>
              تتطلب استجابة فورية - يرجى مراجعة الطلبات أدناه
            </div>
          </div>
          <button
      ...
      <div className="grid grid-cols-4 gap-4">
        {[
          ...
          <div
            key={i}
            className={`bg-white rounded-2xl p-5 border ${s.color} shadow-sm text-center`}
          >
            <div className="text-2xl mb-2">{s.icon}</div>
            <div className="text-[#1E293B]" style={{ fontSize: '24px', fontWeight: 700 }}>
              {s.value}
            </div>
            <div className="text-gray-500" style={{ fontSize: '12px' }}>
              {s.label}
      ...
      <div className="space-y-4">
        {requests.map((req) => {
          ...
          return (
            <div
              key={req.id}
              className={`bg-white rounded-2xl border ${isPending ? config.border : 'border-gray-100'} shadow-sm overflow-hidden transition-all`}
            >
              <div
                className={`h-1 ${isPending ? (req.urgency === 'critical' ? 'bg-red-500' : req.urgency === 'high' ? 'bg-orange-500' : 'bg-yellow-500') : 'bg-gray-200'}`}
              ></div>
              <div className="p-5">
                <div className="flex flex-wrap items-start gap-4">
                  {/* Blood Type */}
                  <div
                    className={`w-14 h-14 rounded-2xl flex items-center justify-center flex-shrink-0 ${isPending ? 'bg-red-50' : 'bg-gray-50'}`}
                  >
                    <span
                      className={`${isPending ? 'text-[#C62828]' : 'text-gray-400'}`}
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex flex-wrap items-center gap-2 mb-2">
                      <h3 className="text-[#1E293B]" style={{ fontSize: '16px', fontWeight: 700 }}>
                        {req.hospital}
                      </h3>
                      <span
                        className={`px-2.5 py-1 rounded-full ${config.bg} ${config.color}`}
                        style={{ fontSize: '12px', fontWeight: 600 }}
                      >
                        {config.label}
                      </span>
                      <span
                        className={`px-2.5 py-1 rounded-full ${isPending ? 'bg-yellow-100 text-yellow-700' : req.status === 'fulfilled' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}
                        style={{ fontSize: '12px', fontWeight: 600 }}
                      >
                        {isPending
                          ? 'قيد الانتظار'
                  ...
                  {!isPending && (
                    <div
                      className={`flex items-center gap-2 ${req.status === 'fulfilled' ? 'bg-green-100' : 'bg-red-100'} px-4 py-2.5 rounded-xl`}
                    >
                      {req.status === 'fulfilled' ? (
                        <>
