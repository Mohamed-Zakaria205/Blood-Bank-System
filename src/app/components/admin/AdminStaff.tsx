...
export default function AdminStaff() {
  ...
  const validate = () => {
    ...
    if (!form.fullName.trim() || form.fullName.trim().split(' ').length < 2) e.fullName = 'أدخل الاسم الثلاثي على الأقل';
    if (!form.nationalId || form.nationalId.length !== 14 || !/^\d+$/.test(form.nationalId)) e.nationalId = 'رقم الهوية يجب أن يكون 14 رقماً';
    if (!form.phone || form.phone.length < 11) e.phone = 'رقم الهاتف يجب أن يكون 11 رقماً على الأقل';
    if (!form.address.trim()) e.address = 'أدخل العنوان';
    if (!form.email.trim() || !form.email.includes('@')) e.email = 'أدخل بريداً إلكترونياً صحيحاً';
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-gray-900" style={{ fontSize: '22px', fontWeight: 800 }}>إدارة الكوادر الطبية</h1>
          <p className="text-gray-500" style={{ fontSize: '14px' }}>{staff.length} حساب مسجل في النظام</p>
        </div>
        <button
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          ...
          <button key={i} onClick={s.onClick} className={`${s.bg} rounded-xl p-4 text-center hover:opacity-80 transition-opacity`}>
          ...
          <table className="w-full min-w-[800px]">
            <thead>
              <tr className="bg-gray-50">
                {['الاسم والدور', 'رقم الهوية', 'الهاتف', 'البريد الإلكتروني', 'العنوان', 'تاريخ الإضافة', 'الحالة', 'إجراءات'].map(h => (
            ...
            <tbody className="divide-y divide-gray-50">
                  ...
                  <tr key={u.id} className="hover:bg-gray-50 transition-colors">
                    <td className="px-4 py-4">
                      <div className="flex items-center gap-3">
                        <div className={`w-10 h-10 ${cfg.bgColor} rounded-full flex items-center justify-center flex-shrink-0`}>
                          <cfg.icon className={`w-4 h-4 ${cfg.color}`} />
                        </div>
                        <div>
                          <p className="text-gray-900" style={{ fontSize: '13px', fontWeight: 600 }}>{u.name}</p>
                          <span className={`inline-block px-2 py-0.5 rounded-full ${cfg.badge}`} style={{ fontSize: '10px', fontWeight: 700 }}>{cfg.label}</span>
                        </div>
                      </div>
                    </td>
                    <td className="px-4 py-4">
                      <span className="text-gray-600 font-mono" style={{ fontSize: '12px' }}>{u.nationalId || '—'}</span>
                    </td>
                    <td className="px-4 py-4">
                      <span className="text-gray-600 font-mono" style={{ fontSize: '12px' }}>{u.phone || '—'}</span>
                    </td>
                    <td className="px-4 py-4">
                      <div className="flex items-center gap-2">
                        <span className="text-gray-600 font-mono" style={{ fontSize: '11px' }}>{u.email}</span>
                        <button onClick={() => copyEmail(u.email)} className="p-1 text-gray-400 hover:text-green-600 hover:bg-green-50 rounded-lg transition-all">
                          {copied === u.email ? <Check className="w-3.5 h-3.5 text-green-600" /> : <Copy className="w-3.5 h-3.5" />}
                        </button>
                      </div>
                    </td>
                    <td className="px-4 py-4">
                      <span className="text-gray-500" style={{ fontSize: '12px' }}>{u.address || '—'}</span>
                    </td>
                    <td className="px-4 py-4">
                      <span className="text-gray-400" style={{ fontSize: '12px' }}>{u.createdAt}</span>
                    </td>
                    <td className="px-4 py-4">
                      <span className={`px-2.5 py-1 rounded-full ${u.status === 'active' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}
                        style={{ fontSize: '11px', fontWeight: 700 }}>
                        {u.status === 'active' ? 'نشط' : 'معطل'}
                      </span>
                    </td>
                    <td className="px-4 py-4">
                      <button onClick={() => setDeleteId(u.id)} className="p-2 text-red-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-all">
              ...
              {filtered.length === 0 && (
                <tr><td colSpan={8} className="py-16 text-center text-gray-400" style={{ fontSize: '14px' }}>لا توجد نتائج</td></tr>
              )}
            </tbody>
          </table>
        </div>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4"
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="flex items-center justify-between p-6 border-b border-gray-100 sticky top-0 bg-white z-10">
              <div>
                <h3 className="text-gray-900" style={{ fontSize: '18px', fontWeight: 700 }}>إضافة كادر طبي جديد</h3>
                <p className="text-gray-500 mt-0.5" style={{ fontSize: '13px' }}>أدخل جميع البيانات المطلوبة</p>
              </div>
            ...
            <div className="p-6 space-y-5">
              {/* Role Selector */}
              <div>
                <label className="block text-gray-700 mb-2" style={{ fontSize: '13px', fontWeight: 600 }}>نوع الحساب *</label>
                <div className="grid grid-cols-2 gap-3">
                  {(['doctor', 'lab', 'inventory'] as StaffRole[]).map(r => {
                    const cfg = roleConfig[r];
                    return (
                      <button key={r} type="button" onClick={() => updateForm('role', r)}
                        ...
                        <div className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 ${form.role === r ? cfg.bgColor : 'bg-gray-100'}`}>
                          <cfg.icon className={`w-5 h-5 ${form.role === r ? cfg.color : 'text-gray-400'}`} />
                        </div>
                        <div className="text-right">
                          <p className={form.role === r ? cfg.color : 'text-gray-600'} style={{ fontSize: '14px', fontWeight: 700 }}>{cfg.label}</p>
                        </div>
                      </button>
                    );
                  })}
              ...
              <div>
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <span className="text-gray-700" style={{ fontSize: '13px', fontWeight: 700 }}>البيانات الشخصية</span>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {/* Full Name */}
                  <div className="sm:col-span-2">
                    <label className="block text-gray-700 mb-1.5" style={{ fontSize: '13px', fontWeight: 600 }}>الاسم الكامل *</label>
                    <input
                      type="text" value={form.fullName} onChange={e => updateForm('fullName', e.target.value)}
                      ...
                      style={{ fontSize: '13px' }} />
                    {errors.fullName && <p className="text-red-500 mt-1" style={{ fontSize: '11px' }}>{errors.fullName}</p>}
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      type="text" value={form.nationalId} onChange={e => updateForm('nationalId', e.target.value.replace(/\D/g, '').slice(0, 14))}
                  ...
                  <div>
                    ...
                    <input
                      type="text" value={form.address} onChange={e => updateForm('address', e.target.value)}
              ...
              <div>
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <span className="text-gray-700" style={{ fontSize: '13px', fontWeight: 700 }}>بيانات الحساب</span>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {/* Email */}
                  <div>
                    <label className="block text-gray-700 mb-1.5" style={{ fontSize: '13px', fontWeight: 600 }}>البريد الإلكتروني *</label>
                    <input
                  ...
                  {/* Password */}
                  <div>
                    <label className="block text-gray-700 mb-1.5" style={{ fontSize: '13px', fontWeight: 600 }}>كلمة المرور *</label>
                    <div className="relative">
                      <input
                    ...
                    {errors.password && <p className="text-red-500 mt-1" style={{ fontSize: '11px' }}>{errors.password}</p>}
                  </div>
                </div>
              </div>
            </div>
      ...
      {deleteId && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl p-6 max-w-sm w-full text-center">
            ...
            <h3 className="text-gray-900 mb-2" style={{ fontSize: '18px', fontWeight: 700 }}>حذف الحساب</h3>
            <p className="text-gray-500 mb-6" style={{ fontSize: '14px' }}>هل أنت متأكد من حذف هذا الحساب؟ لا يمكن التراجع.</p>
            <div className="flex gap-3">
              <button onClick={() => setDeleteId(null)} className="flex-1 py-2.5 border border-gray-200 text-gray-600 rounded-xl hover:bg-gray-50 transition-all" style={{ fontSize: '14px', fontWeight: 600 }}>إلغاء</button>
              <button onClick={() => deleteUser(deleteId)} className="flex-1 py-2.5 bg-red-600 hover:bg-red-700 text-white rounded-xl transition-all" style={{ fontSize: '14px', fontWeight: 600 }}>حذف</button>
            </div>
          </div>
        </div>
      )}
