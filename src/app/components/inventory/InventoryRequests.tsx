...
export default function InventoryRequests() {
  ...
  const [filterStatus, setFilterStatus] = useState<
    "all" | "pending" | "approved" | "fulfilled"
  >("all");
  ...
  const filtered = requests.filter((r) => {
    ...
    const matchSearch =
      r.hospitalName.includes(search) || r.bloodType.includes(search);
  ...
  const compatibleBags = currentReq
    ? getCompatibleBags(currentReq.bloodType)
    : [];
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg p-6">
            ...
            <p className="text-gray-500 mb-4" style={{ fontSize: "13px" }}>
              مطلوب: {currentReq.quantity} وحدة {currentReq.bloodType} — حدد
              الحقائب المناسبة
            </p>
            {selectedBags.length > 0 && (
              <div className="mb-3 p-2 bg-green-50 border border-green-100 rounded-xl">
                ...
                >
                  ...
                  {selectedBags.length >= currentReq.quantity &&
                    " ✓ جاهز للصرف"}
                </p>
              </div>
            )}
              ...
              {compatibleBags.length === 0 ? (
                ...
                compatibleBags.map((bag) => {
                  const days = Math.ceil(
                    (new Date(bag.expiryDate).getTime() -
                      new Date("2025-04-29").getTime()) /
                      (1000 * 60 * 60 * 24),
                  );
                  return (
                    <label
            ...
            <div className="flex gap-3">
              <button
                ...
                disabled={
                  selectedBags.length === 0 || fulfillRequestMutation.isPending
                }
            ...
            <div className="space-y-4">
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setNewReq((p) => ({ ...p, hospitalName: e.target.value }))
                  }
              ...
              <div>
                ...
                <input
                  value={newReq.notes}
                  onChange={(e) =>
                    setNewReq((p) => ({ ...p, notes: e.target.value }))
                  }
                  placeholder="اختياري"
                  className="w-full px-4 py-2.5 border border-gray-200 rounded-xl bg-gray-50 text-gray-900 outline-none focus:border-green-400"
                  style={{ fontSize: "13px" }}
