...
export default function InventoryBags() {
  ...
  const availableCount = bags.filter(b => b.status === 'available' && daysUntil(b.expiryDate) >= 0).length;
  const expiredCount   = bags.filter(b => b.status === 'available' && daysUntil(b.expiryDate) <  0).length;
  ...
  const getBagStatus = (bag: BloodBag) => {
    ...
    if (bag.status === 'available' && days <  0) return { label: 'منتهية', cls: 'bg-red-100 text-red-700',    isExpired: true,  isAvailable: false };
    if (bag.status === 'available')              return { label: 'متاح',   cls: 'bg-green-100 text-green-700', isExpired: false, isAvailable: true  };
    if (bag.status === 'issued')                 return { label: 'مُصدَّر', cls: 'bg-blue-100 text-blue-700',  isExpired: false, isAvailable: false };
    if (bag.status === 'rejected')               return { label: 'مرفوض', cls: 'bg-orange-100 text-orange-700', isExpired: false, isAvailable: false };
    return { label: bag.status, cls: 'bg-gray-100 text-gray-500', isExpired: false, isAvailable: false };
  };
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-gray-900" style={{ fontSize: '22px', fontWeight: 800 }}>حقائب الدم</h1>
        <p className="text-gray-500" style={{ fontSize: '14px' }}>جرد شامل لجميع الحقائب المخزنة</p>
      </div>

      {/* Stats */}
      <div className="grid grid-cols-3 gap-4">
        {[
          { label: 'متاحة وصالحة',    value: availableCount, color: 'text-green-700', bg: 'bg-green-50 border-green-200', filter: 'available'    as const },
          { label: 'منتهية الصلاحية', value: expiredCount,   color: 'text-red-700',   bg: 'bg-red-50 border-red-200',     filter: 'expired_only' as const },
          { label: 'مُصدَّرة',         value: issuedCount,    color: 'text-blue-700',  bg: 'bg-blue-50 border-blue-200',   filter: 'all'          as const },
        ].map(s => (
          <button key={s.label}
            ...
            <div className={`${s.color} opacity-80`} style={{ fontSize: '12px', fontWeight: 600 }}>{s.label}</div>
          </button>
        ))}
      </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-green-800" style={{ fontSize: '14px', fontWeight: 700 }}>
                {selectedBagIds.length} {selectedBagIds.length === 1 ? 'حقيبة محددة' : 'حقائب محددة للتصدير'}
              </p>
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead>
              <tr className="bg-gray-50">
                ...
                {['كود الحقيبة', 'الفصيلة', 'نوع الدم', 'تاريخ التسجيل', 'تاريخ الانتهاء', 'الحالة', 'إجراء'].map(h => (
                  <th key={h} className="px-4 py-3 text-right text-gray-500"
                    style={{ fontSize: '12px', fontWeight: 600 }}>{h}</th>
                ))}
              </tr>
            </thead>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1.5">
                        <span className="font-mono text-green-600 bg-green-50 px-2 py-0.5 rounded"
                          style={{ fontSize: '11px', fontWeight: 700 }}>{bag.bagCode}</span>
                        {(isExpired || isNear) &&
                          <AlertTriangle className={`w-3.5 h-3.5 ${isExpired ? 'text-red-500' : 'text-orange-400'}`} />}
                      </div>
                    </td>
                    <td className="px-4 py-3">
                      <span className="px-2 py-0.5 bg-red-50 text-red-600 rounded"
                        style={{ fontSize: '12px', fontWeight: 800 }}>{bag.bloodType}</span>
                    </td>
                    <td className="px-4 py-3 text-gray-600" style={{ fontSize: '12px' }}>{donTypeLabels[bag.donationType]}</td>
                    <td className="px-4 py-3 text-gray-600" style={{ fontSize: '12px' }}>{bag.collectedDate}</td>
                    <td className="px-4 py-3">
                      <span className={`${isExpired ? 'text-red-600' : isNear ? 'text-orange-500' : 'text-gray-600'}`}
                        style={{ fontSize: '12px', fontWeight: (isExpired || isNear) ? 700 : 400 }}>
                        {bag.expiryDate}
                        {isExpired && <span className="mr-1" style={{ fontSize: '10px' }}>(منتهية)</span>}
                        {isNear    && <span className="mr-1" style={{ fontSize: '10px' }}>({days}د)</span>}
                      </span>
                    </td>
                    <td className="px-4 py-3">
                      <span className={`px-2.5 py-0.5 rounded-full ${cls}`}
                        style={{ fontSize: '11px', fontWeight: 700 }}>{label}</span>
                    </td>
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1.5">
                        {isAvailable && (
                          <button onClick={() => openExportForBag(bag)}
                        ...
                        {(isAvailable || isExpired || bag.status === 'rejected') && (
                          <button onClick={() => setDisposeModal(bag)}
          ...
          {displayBags.length === 0 && (
            <div className="py-14 text-center">
              ...
              <p className="text-gray-400" style={{ fontSize: '14px' }}>لا توجد حقائب</p>
            </div>
          )}
        </div>
      </div>
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-gray-900" style={{ fontSize: '17px', fontWeight: 700 }}>
                    تصدير {selectedBagsData.length > 1 ? `${selectedBagsData.length} حقائب` : 'حقيبة دم'}
                  </h3>
                  <p className="text-gray-500" style={{ fontSize: '12px' }}>
                    {exportStep === 1 ? 'أدخل بيانات المستلِم' : 'مراجعة وتأكيد نهائي'}
                  </p>
                </div>
              </div>
              <div className="flex items-center gap-3">
                ...
                <div className="flex items-center gap-1.5">
                  <div className="w-6 h-6 rounded-full flex items-center justify-center text-white"
                    style={{ fontSize: '11px', fontWeight: 700, backgroundColor: exportStep >= 1 ? '#16a34a' : '#d1d5db' }}>1</div>
                  <div className={`w-6 h-0.5 ${exportStep >= 2 ? 'bg-green-600' : 'bg-gray-200'}`} />
                  <div className="w-6 h-6 rounded-full flex items-center justify-center text-white"
                    style={{ fontSize: '11px', fontWeight: 700, backgroundColor: exportStep >= 2 ? '#16a34a' : '#d1d5db' }}>2</div>
                </div>
            ...
            <div className="px-6 pt-4">
              <div className="p-3 bg-gray-50 rounded-xl border border-gray-100">
                <p className="text-gray-400 mb-2" style={{ fontSize: '11px', fontWeight: 600 }}>الحقائب المحددة</p>
                <div className="flex flex-wrap gap-1.5">
                  {selectedBagsData.map(bag => (
                    <span key={bag.id}
                      ...
                      <span className="font-mono text-green-600" style={{ fontSize: '11px', fontWeight: 700 }}>{bag.bagCode}</span>
                      <span className="px-1 py-0.5 bg-red-50 text-red-600 rounded" style={{ fontSize: '10px', fontWeight: 800 }}>{bag.bloodType}</span>
                    </span>
                  ))}
                </div>
              </div>
              ...
              <div className="p-6 space-y-4">
                <div>
                  ...
                  <input
                    ...
                    onChange={e => setExportForm(p => ({ ...p, recipientName: e.target.value }))}
                  ...
                  {exportErrors.recipientName &&
                    <p className="text-red-500 mt-1" style={{ fontSize: '11px' }}>{exportErrors.recipientName}</p>}
                </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <input
                      ...
                      onChange={e => setExportForm(p => ({ ...p, nationalId: e.target.value }))}
                      placeholder="14 رقم"
                      className={`w-full px-4 py-2.5 border rounded-xl bg-gray-50 text-gray-900 outline-none focus:border-green-400
                        ${exportErrors.nationalId ? 'border-red-300' : 'border-gray-200'}`}
                      style={{ fontSize: '13px' }} />
                    {exportErrors.nationalId &&
                      <p className="text-red-500 mt-1" style={{ fontSize: '11px' }}>{exportErrors.nationalId}</p>}
                  </div>
                  <div>
                    <label className="block text-gray-700 mb-1.5" style={{ fontSize: '13px', fontWeight: 600 }}>رقم الهاتف</label>
                    <input
                      value={exportForm.phone}
                      onChange={e => setExportForm(p => ({ ...p, phone: e.target.value }))}
                ...
                <div>
                  <label className="block text-gray-700 mb-1.5" style={{ fontSize: '13px', fontWeight: 600 }}>سبب التصدير *</label>
                  <textarea
                    value={exportForm.reason}
                    onChange={e => setExportForm(p => ({ ...p, reason: e.target.value }))}
                    rows={2} placeholder="مثال: نقل دم بعد عملية جراحية"
                    ...
                    style={{ fontSize: '13px' }} />
                  {exportErrors.reason &&
                    <p className="text-red-500 mt-1" style={{ fontSize: '11px' }}>{exportErrors.reason}</p>}
                </div>

                <div className="p-3 bg-green-50 border border-green-100 rounded-xl">
                  <p className="text-green-700" style={{ fontSize: '11px' }}>
                    📋 سيتم تسجيل هذا التصدير تلقائياً باسم: <strong>{getCurrentUserName()}</strong> مع التاريخ والوقت
                  </p>
                </div>
              </div>
            )}
            ...
            {exportStep === 2 && (
              <div className="p-6 space-y-4">
                <div className="flex items-start gap-3 p-4 bg-amber-50 border border-amber-200 rounded-xl">
                  ...
                  <div>
                    <p className="text-amber-800" style={{ fontSize: '13px', fontWeight: 600 }}>مراجعة بيانات التصدير</p>
                    <p className="text-amber-600" style={{ fontSize: '11px', marginTop: '2px' }}>
                      يُرجى التحقق من صحة جميع البيانات قبل التأكيد النهائي. لا يمكن التراجع عن هذه العملية.
                    </p>
                  </div>
                </div>
                ...
                <div className="bg-gray-50 rounded-xl border border-gray-100 divide-y divide-gray-100">
                  {[
                    { icon: '👤', label: 'اسم المستلِم',   value: exportForm.recipientName },
                    { icon: '🪪', label: 'الرقم القومي',   value: exportForm.nationalId },
                    { icon: '📞', label: 'رقم الهاتف',     value: exportForm.phone || '—' },
                    { icon: '📋', label: 'سبب التصدير',    value: exportForm.reason },
                    { icon: '🩸', label: 'عدد الحقائب',    value: `${selectedBagsData.length} حقيبة` },
              ...
              {exportStep === 1 ? (
                ...
                <>
                  ...
                  <button onClick={() => setExportStep(1)}
                    ...
                    style={{ fontSize: '14px', fontWeight: 600 }}>رجوع</button>
                </>
              )}
            </div>
          </div>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6">
            <div className="flex items-center gap-3 mb-5">
              ...
              <div>
                <h3 className="text-gray-900" style={{ fontSize: '17px', fontWeight: 700 }}>تأكيد الإتلاف</h3>
                <p className="text-gray-500" style={{ fontSize: '12px' }}>{disposeModal.bagCode} — فصيلة {disposeModal.bloodType}</p>
              </div>
            </div>
            ...
            <div className="mb-5">
              <label className="block text-gray-700 mb-1.5" style={{ fontSize: '13px', fontWeight: 600 }}>سبب الإتلاف</label>
              <textarea value={disposeReason} onChange={e => setDisposeReason(e.target.value)} rows={2}
