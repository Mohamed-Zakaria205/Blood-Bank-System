...
export default function InventoryDashboard() {
  ...
  BLOOD_TYPES.forEach((t) => {
    byType[t] = bags.filter(
      (b) => b.bloodType === t && b.status === "available",
    ).length;
  });
  ...
  const expiredRatio =
    totalBags > 0
      ? Math.round(((expired + totalDisposed) / totalBags) * 100)
      : 0;
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-gray-500 mt-0.5" style={{ fontSize: "14px" }}>
          مرحباً {user?.name?.split(" ").slice(1, 3).join(" ")} — الثلاثاء، 29
          أبريل 2025
        </p>
      </div>
        ...
        <div className="space-y-2">
          ...
          {nearExpiry.length > 0 && (
            <div className="flex items-center gap-3 p-4 bg-orange-50 border border-orange-200 rounded-2xl">
              ...
              <p
                className="text-orange-700 flex-1"
