...
const SCREENING_TESTS = [
  { key: 'hepatitisC', label: 'التهاب الكبد الوبائي C', abbr: 'HCV',      desc: 'Hepatitis C Virus' },
  { key: 'hepatitisB', label: 'التهاب الكبد الوبائي B', abbr: 'HBV',      desc: 'Hepatitis B Virus' },
  { key: 'syphilis',   label: 'مرض الزهري',              abbr: 'Syphilis', desc: 'Treponema Pallidum' },
  { key: 'hiv',        label: 'فيروس نقص المناعة',       abbr: 'HIV',      desc: 'Human Immunodeficiency Virus (AIDS)' },
];
...
function buildCombinedList() {
  const completed = initialResults.map(r => ({
    ...
    hepatitisB: r.hepatitisB,
    hepatitisC: r.hepatitisC,
    syphilis: r.syphilis,
    hiv: r.hiv,
  ...
  const pendingEntries = samples
    ...
    .map(s => ({
      ...
      hepatitisB: null as any,
      hepatitisC: null as any,
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl overflow-hidden">
        ...
        <div className="p-5 flex items-center justify-between" style={{ background: headerGradient }}>
          <div>
            <h3 className="text-white" style={{ fontSize: '17px', fontWeight: 700 }}>تفاصيل نتائج الفحص</h3>
        ...
        <div className="p-5 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="bg-green-50 border border-green-100 rounded-xl p-3">
              <div className="flex items-center gap-1.5 mb-1">
                ...
                <div className="text-green-600" style={{ fontSize: '10px', fontWeight: 600 }}>كود العينة</div>
              </div>
              <div className="font-mono text-green-800" style={{ fontSize: '12px', fontWeight: 800 }}>{entry.sampleCode}</div>
            </div>
            <div className="bg-gray-50 rounded-xl p-3">
              <div className="flex items-center gap-1.5 mb-1">
                <CreditCard className="w-3 h-3 text-gray-400" />
                <div className="text-gray-400" style={{ fontSize: '10px' }}>رقم الهوية</div>
              </div>
              <div className="font-mono text-gray-900" style={{ fontSize: '12px', fontWeight: 700 }}>{entry.nationalId}</div>
            </div>
          </div>
          ...
          <div className={`flex items-center gap-3 p-4 rounded-2xl border ${
            isPending ? 'bg-yellow-50 border-yellow-100'
              : isSafe ? 'bg-green-50 border-green-100'
              : 'bg-red-50 border-red-100'
          }`}>
            {isPending ? (
              <><Clock className="w-8 h-8 text-yellow-500 flex-shrink-0" />
                <div>
                  <div className="text-yellow-700" style={{ fontSize: '16px', fontWeight: 700 }}>⏳ معلق</div>
                  <div className="text-yellow-600" style={{ fontSize: '13px' }}>لم يتم الفحص بعد</div>
                </div></>
            ) : isSafe ? (
              <><CheckCircle2 className="w-8 h-8 text-green-600 flex-shrink-0" />
                <div>
                  <div className="text-green-700" style={{ fontSize: '16px', fontWeight: 700 }}>الدم آمن ✓</div>
                  <div className="text-green-600" style={{ fontSize: '13px' }}>
                    جميع الفحوصات سالبة — مقبول
                    {entry.confirmedBloodType && <> • فصيلة: <strong>{entry.confirmedBloodType}</strong></>}
                  </div>
                </div></>
            ) : (
              <><XCircle className="w-8 h-8 text-red-600 flex-shrink-0" />
                <div>
                  <div className="text-red-700" style={{ fontSize: '16px', fontWeight: 700 }}>الدم مرفوض ✗</div>
            ...
            <div>
              ...
              <div className="space-y-2">
                {SCREENING_TESTS.map(test => {
                  ...
                  return (
                    <div key={test.key} className={`flex items-center justify-between px-3 py-2.5 rounded-xl border ${isPositive ? 'bg-red-50 border-red-100' : 'bg-gray-50 border-gray-100'}`}>
                      <div className="flex items-center gap-2.5">
                        <div className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 ${isPositive ? 'bg-red-100' : 'bg-green-50'}`}>
                          <span className={isPositive ? 'text-red-700' : 'text-green-700'} style={{ fontSize: '8px', fontWeight: 900 }}>{test.abbr}</span>
                        </div>
                        <div>
          ...
          {entry.notes && (
            <div className="bg-yellow-50 border border-yellow-100 rounded-xl p-3">
              <div className="text-yellow-700" style={{ fontSize: '13px' }}>
                <span style={{ fontWeight: 600 }}>ملاحظات: </span>{entry.notes}
              </div>
            </div>
          )}
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="bg-gray-50 rounded-xl p-3 text-center">
              <div className="text-gray-400" style={{ fontSize: '10px' }}>طبيب المختبر</div>
              <div className="text-gray-800" style={{ fontSize: '12px', fontWeight: 600 }}>{entry.labDoctor}</div>
            </div>
            <div className="bg-gray-50 rounded-xl p-3 text-center">
              <div className="text-gray-400" style={{ fontSize: '10px' }}>التاريخ</div>
              <div className="text-gray-800" style={{ fontSize: '12px', fontWeight: 600 }}>{entry.date}</div>
            </div>
          </div>
...
export default function LabResults() {
  ...
  const getStatusBadge = (entry: CombinedEntry) => {
    if (entry.displayStatus === 'pending') return (
      <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full whitespace-nowrap"
        style={{ background: 'rgba(251,191,36,0.12)', border: '1px solid rgba(251,191,36,0.3)', fontSize: '10px', fontWeight: 700, color: '#92400e' }}>
        <Clock className="w-3 h-3" />معلق
      </span>
    );
    if (entry.displayStatus === 'safe') return (
      <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full whitespace-nowrap"
        style={{ background: 'rgba(74,222,128,0.1)', border: '1px solid rgba(74,222,128,0.25)', fontSize: '10px', fontWeight: 700, color: '#14532d' }}>
        <CheckCircle2 className="w-3 h-3" />آمن
      </span>
    );
    return (
      <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full whitespace-nowrap"
        style={{ background: 'rgba(248,113,113,0.1)', border: '1px solid rgba(248,113,113,0.25)', fontSize: '10px', fontWeight: 700, color: '#7f1d1d' }}>
  ...
  const testBadge = (val: 'negative' | 'positive' | null) => {
    if (!val) return <span className="text-yellow-400" style={{ fontSize: '14px' }}>—</span>;
    return (
      <span className={`inline-flex items-center justify-center w-6 h-6 rounded-full ${val === 'negative' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-600'}`}
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Page Header */}
      <div>
        <h1 className="text-gray-900" style={{ fontSize: '22px', fontWeight: 700 }}>نتائج الفحوصات</h1>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {[
          ...
          { label: 'معلقة', value: totals.pending, icon: '⏳', btn: 'pending', bg: '', style: { background: 'rgba(251,191,36,0.08)', borderColor: 'rgba(251,191,36,0.25)' } },
          { label: 'آمنة', value: totals.safe, icon: '✅', btn: 'safe', bg: '', style: { background: 'rgba(74,222,128,0.08)', borderColor: 'rgba(74,222,128,0.25)' } },
          { label: 'مرفوضة', value: totals.unsafe, icon: '❌', btn: 'unsafe', bg: '', style: { background: 'rgba(248,113,113,0.08)', borderColor: 'rgba(248,113,113,0.25)' } },
        ].map((s, i) => (
          <button key={i} onClick={() => setFilter(s.btn)}
            ...
            <div className="text-gray-900" style={{ fontSize: '24px', fontWeight: 700 }}>{s.value}</div>
      ...
      <div className="bg-gradient-to-br from-green-50 to-emerald-50 border border-green-100 rounded-2xl p-4">
        <div className="flex items-center gap-2 mb-3">
          ...
          <span className="text-green-800" style={{ fontSize: '13px', fontWeight: 700 }}>الفحوصات المعيارية — 4 تحاليل</span>
        </div>
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
          {SCREENING_TESTS.map((t, i) => (
            <div key={t.key} className="bg-white rounded-xl px-3 py-2.5 border border-green-100 flex items-center gap-2.5">
              <div className="w-7 h-7 bg-green-600 rounded-lg flex items-center justify-center flex-shrink-0">
                <span className="text-white" style={{ fontSize: '8px', fontWeight: 900 }}>{i + 1}</span>
              </div>
              <div>
                <p className="text-green-700" style={{ fontSize: '12px', fontWeight: 700 }}>{t.abbr}</p>
                <p className="text-gray-500" style={{ fontSize: '10px' }}>{t.label}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="flex flex-wrap gap-3">
        <div className="flex items-center gap-2 px-3 py-2 rounded-xl border"
          ...
          <span className="text-yellow-700" style={{ fontSize: '12px', fontWeight: 600 }}>معلق — لم يُفحص بعد</span>
        </div>
        <div className="flex items-center gap-2 px-3 py-2 rounded-xl border"
          style={{ background: 'rgba(248,113,113,0.08)', borderColor: 'rgba(248,113,113,0.25)' }}>
          <div className="w-3 h-3 rounded-full bg-red-500" />
          <span className="text-red-700" style={{ fontSize: '12px', fontWeight: 600 }}>مرفوض — نتيجة إيجابية</span>
        </div>
        <div className="flex items-center gap-2 px-3 py-2 rounded-xl border"
          style={{ background: 'rgba(74,222,128,0.08)', borderColor: 'rgba(74,222,128,0.25)' }}>
          <div className="w-3 h-3 rounded-full bg-green-500" />
          <span className="text-green-700" style={{ fontSize: '12px', fontWeight: 600 }}>آمن — جميع الفحوصات سالبة</span>
        </div>
      </div>
            ...
            <thead>
              <tr className="border-b border-gray-100 bg-gray-50/80">
                <th className="text-right px-4 py-3.5 text-gray-500 whitespace-nowrap" style={{ fontSize: '11px', fontWeight: 700 }}>
                  <span className="flex items-center gap-1.5"><FlaskConical className="w-3 h-3" />كود العينة</span>
                </th>
                <th className="text-right px-3 py-3.5 text-gray-500 whitespace-nowrap" style={{ fontSize: '11px', fontWeight: 600 }}>
                  <span className="flex items-center gap-1"><CreditCard className="w-3 h-3" />رقم الهوية</span>
                </th>
                ...
                <tr key={entry.id} className="border-b border-gray-50 hover:bg-gray-50/60 transition-colors"
                  ...
                  <td className="px-4 py-3.5">
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className="font-mono text-green-700 bg-green-50 px-2 py-0.5 rounded-lg border border-green-100"
                        style={{ fontSize: '11px', fontWeight: 700 }}>
                        {entry.sampleCode}
                      </span>
                    </div>
                  ...
                  <td className="px-3 py-3.5">
                    <div>
                      <div className="text-gray-900" style={{ fontSize: '12px', fontWeight: 600 }}>{entry.donorName}</div>
                      <div className="font-mono text-gray-400" style={{ fontSize: '10px' }}>{entry.donorCode}</div>
                    </div>
                  </td>
                  <td className="px-3 py-3.5">
                    <span className="bg-gray-100 text-gray-700 px-2 py-0.5 rounded-lg font-mono"
          ...
          {filtered.length === 0 && (
            <div className="py-16 text-center">
              ...
              <p className="text-gray-400" style={{ fontSize: '14px' }}>
                {search ? `لا توجد نتائج لـ "${search}"` : 'لا توجد نتائج في هذه الفئة'}
              </p>
              {search && (
                <button onClick={() => setSearch('')} className="mt-2 text-green-600 hover:underline"
                  style={{ fontSize: '12px' }}>
                  مسح البحث
                </button>
              )}
